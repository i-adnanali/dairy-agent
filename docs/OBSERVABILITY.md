...
- ~~No decision yet on eval/regression tooling (golden-dataset assertions on tool-call sequences) — that's Cycle 2~~ **Resequenced.** Cycle 2 became the multi-agent system (vendor/sales agent + dispatcher + reconciliation; see [MULTI_AGENT.md](MULTI_AGENT.md), tagged `v0.5.0`). Eval/regression tooling is now Cycle 3, deliberately following the multi-agent work so it can be scoped against a shape that has stabilized rather than one still in flux. Its doc (`REGRESSION.md`) is authored when Cycle 3 begins. It will still only need to assert against one transport's AG-UI event sequences, since there remains a single path through the app.
